...
//
//	  see License below for conditions of use.
// _______________________________________________________________________________
//
...
skip:
	...
	if(IsConnected()) {
		...
		if(CallbackTriggerFlags & WIIMOTE_CONNECTED)
			{
			ChangedNotifier(WIIMOTE_CONNECTED, Internal);
			if(ChangedCallback)
				ChangedCallback(*this, WIIMOTE_CONNECTED, Internal);
			}
		return true;
		}
